<link rel="stylesheet" href="assets/css/includes/footer.css">
<div class="benefits-section">
  <div class="benefits-container">
    <div class="benefits-icon-container">
      <img loading="lazy" src="assets/img/svg/shipping.svg" class="benefits-icon" alt="shipping">
      <div class="benefits-text">INTERNATIONAL SHIPPING</div>
    </div>
    <div class="benefits-icon-container">
      <img loading="lazy" src="assets/img/svg/warranty.svg" class="benefits-icon" alt="warranty">
      <div class="benefits-text">6 month warranty</div>
    </div>
    <div class="benefits-icon-container">
      <img loading="lazy" src="assets/img/svg/safe.svg" class="benefits-icon" alt="safe">
      <div class="benefits-text">safe shopping</div>
    </div>
    <div class="benefits-icon-container">
      <img loading="lazy" src="assets/img/svg/return.svg" class="benefits-icon" alt="return">
      <div class="benefits-text">FREE RETURNS</div>
    </div>
  </div>
</div>
<div class="footer-section">
  <div class="footer-container">
    <div class="footer-inner">
      <div class="footer-logo-container">
        <div class="column-15">
          <div class="footer-logo-inner">
            <img loading="lazy" src="assets/img/svg/logo-black.svg" class="footer-logo" alt="logo-black">
            <div class="footer-slogan-text">The bright side of socks</div>
          </div>
        </div>
        <div class="column-16">
          <div class="social-media-container">
            <div class="social-media-inner">
              <div class="social-media-item">Instagram</div>
              <div class="social-media-item">FACEBOOK</div>
              <div class="social-media-item">Twitter</div>
            </div>
          </div>
        </div>
        <div class="column-17">
          <div class="copyright-container">
            <div class="copyright-inner">
              <div class="copyright-light">© 2023</div>
              <div class="copyright-bold">SUNNY</div>
              <div class="copyright-light">,</div>
            </div>
            <div class="copyright-light">All Rights Reserved</div>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>