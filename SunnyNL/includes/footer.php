<link rel="stylesheet" href="assets/css/includes/footer.css">
<div class="benefits-section">
      <div class="benefits-container">
        <div class="benefits-icon-container">
          <img
            loading="lazy"
            src="assets/img/svg/shipping.svg"
            class="benefits-icon"
          />
          <div class="benefits-text">INTERNATIONALE VERZENDING</div>
        </div>
        <div class="benefits-icon-container">
          <img
            loading="lazy"
            src="assets/img/svg/warranty.svg"
            class="benefits-icon"
          />
          <div class="benefits-text">6 maanden garantie</div>
        </div>
        <div class="benefits-icon-container">
          <img
            loading="lazy"
            src="assets/img/svg/safe.svg"
            class="benefits-icon"
          />
          <div class="benefits-text">veilig winkelen</div>
        </div>
        <div class="benefits-icon-container">
          <img
            loading="lazy"
            src="assets/img/svg/return.svg"
            class="benefits-icon"
          />
          <div class="benefits-text">gratis retour</div>
        </div>
      </div>
    </div>
    <div class="footer-section">
      <div class="footer-container">
        <div class="footer-inner">
          <div class="footer-logo-container">
            <div class="column-15">
              <div class="footer-logo-inner">
                <img
                  loading="lazy"
                  src="assets/img/svg/logo-black.svg"
                  class="footer-logo"
                />
                <div class="footer-slogan-text">The bright side of socks</div>
              </div>
            </div>
            <div class="column-16">
              <div class="social-media-container">
                <div class="social-media-inner">
                  <div class="social-media-item">Instagram</div>
                  <div class="social-media-item">FACEBOOK</div>
                  <div class="social-media-item">Twitter</div>
                </div>
              </div>
            </div>
            <div class="column-17">
              <div class="copyright-container">
                <div class="copyright-inner">
                  <div class="copyright-light">© 2023</div>
                  <div class="copyright-bold">SUNNY</div>
                  <div class="copyright-light">,</div>
                </div>
                <div class="copyright-light">Alle rechten voorbehouden</div>
              </div>
            </div>
          </div> 
        </div>
      </div>
    </div>
  </div>
</div>